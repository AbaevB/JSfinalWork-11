...
> Для удаления товара из корзины предусмотрена кнопка с классом .**basket__close**. При нажатии на неё товар удаляется из корзины (удалить соответствующий элемент из DOM с помощью метода **.remove()**).
...
Функционал этого задания аналогичен тому, которые  реализовывали при отрисовке основных карточек на странице. Отличие только в том, что вам в необходимо найти общем массиве карточек нужную, взять из неё данные и на основе готовой разметки отрисовать карточки в корзине.
...
Примечание: Это один из путей реализации функционала каорзины , возможны и другие.
